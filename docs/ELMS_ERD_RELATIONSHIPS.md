...
| `department` | ⚠️ REDUNDANT TEXT COLUMN — stores department name as plain text. This is a leftover from an earlier version. The actual relationship is handled by `department_id`. |
| `position` | ⚠️ REDUNDANT TEXT COLUMN — same issue as above. Actual relationship is `position_id`. |
...
**Eloquent method in `Employee` model:** `belongsTo(Department::class, 'department_id')` — aliased as `departmentRecord` to avoid conflict with the redundant `department` text column.
...
None. `leave_applications` is a leaf table — nothing else points to it.
...
## What this table is
The `system_notifications` table stores every in-app notification generated by the system. Instead of using email, the system creates rows here to notify users about important events — account activation, leave submission confirmation, and leave approval or rejection decisions. Users see these through the notification bell in the header.
...
| `read_at` | Timestamp of when the user read the notification. Null = unread. The unread count badge in the header counts rows where `read_at IS NULL`. |
