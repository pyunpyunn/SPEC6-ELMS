                ...
                <tbody>
                    @forelse($leaveApplications as $leave)
                        <tr>
                            ...
                            <td>{{ $leave->status === 'pending' ? '-' : 'Company Reviewer' }}</td>
                            <td>
                                @if($leave->status === 'pending')
                                    <form method="POST" action="{{ route('employee.leave.cancel', $leave) }}" onsubmit="return confirm('Cancel this pending leave request?')">
                                        @csrf
                                        @method('DELETE')
