...
<div class="page active" id="page-requests">
    <div class="page-header">
        ...
        <div class="page-actions">
            <a class="btn btn-outline btn-sm" href="{{ route('hr.reports.export', request()->all() + ['type' => 'leaves']) }}">Export CSV</a>
        </div>
    </div>

    <div class="dept-log-tabs">
        <a class="dept-log-tab {{ ! $selectedDepartment ? 'active' : '' }}" href="{{ route('hr.requests.index') }}">All Departments</a>
        @foreach($departments as $department)
            <a class="dept-log-tab {{ $selectedDepartment?->id === $department->id ? 'active' : '' }}" href="{{ route('hr.requests.index', ['department_id' => $department->id]) }}">{{ $department->name }}</a>
        @endforeach
    </div>
            ...
            <tbody id="reqTableBody">
                @forelse($requests as $leave)
                    <tr data-status="{{ $leave->status }}" data-dept="{{ $leave->employee->departmentRecord?->name }}" data-type="{{ str($leave->leaveType->name)->slug() }}">
                        ...
                        <td>
                            <button class="btn btn-outline btn-sm" type="button" onclick="openReviewModal(@js([
                                'id' => $leave->id,
                                ...
                                'review_action' => route('hr.requests.review', $leave),
                            ]))">{{ $leave->status === 'pending' ? 'Review' : 'View' }}</button>
                        </td>
                    </tr>
                @empty
