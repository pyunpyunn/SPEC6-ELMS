...
<div class="page active" id="page-approvals">
    <div class="page-header">
        <div><h1>Approval Inbox</h1><p>Search, filter, and review team leave requests.</p></div>
    </div>
    ...
    <form class="filter-bar" method="GET">
        ...
        <div class="filter-bar-row">
            ...
            <select name="status">
                ...
                @foreach(['pending','approved','rejected','cancelled'] as $status)<option value="{{ $status }}" @selected(request('status') === $status)>{{ ucfirst($status) }}</option>@endforeach
            </select>
        ...
        <table>
            ...
            @forelse($requests as $leave)
                @php($reviewData = [
                    ...
                    'review_action' => route('manager.approvals.review', $leave),
                    'is_pending' => $leave->status === 'pending',
                ])
                <tr>
                    <td><div class="td-name">{{ $leave->employee->full_name }}</div><div class="td-sub">{{ $leave->employee->employee_id }}</div></td>
                    <td><div class="td-name">{{ $leave->employee->departmentRecord?->name ?? '-' }}</div><div class="td-sub">{{ $leave->employee->position ?? '-' }}</div></td>
    ...
    <div class="pagination">{{ $requests->links('vendor.pagination.hr', ['anchor' => 'page-approvals']) }}</div>
</div>
...
function openReviewModal(data) {
    ...
    document.getElementById('reviewStatusBadge').innerHTML = '<span class="badge badge-' + statusLower + '">' + (data.status || 'Pending') + '</span>';
    const pending = data.is_pending === true;
    document.getElementById('reviewIsPending').value = pending ? '1' : '0';
    document.getElementById('reviewPendingActions').style.display = pending ? 'flex' : 'none';
    ...
    document.getElementById('reviewExistingRemarks').textContent = data.remarks || '-';
    ...
    if (data.proof) {
        ...
        previewImg.src = isImage ? '{{ asset('storage') }}/' + data.proof : '';
