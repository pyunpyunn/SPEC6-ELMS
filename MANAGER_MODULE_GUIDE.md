...
| `/manager/requests` | `manager.requests.index` | Approval inbox for team leave requests |
| `/manager/requests/{leaveApplication}` | `manager.requests.show` | Leave request review screen |
| `/manager/requests/{leaveApplication}/review` | `manager.requests.review` | Approve/reject a pending team leave request |
...
| `resources/views/manager/requests.blade.php` | Approval inbox |
...
- The UI follows the `kuan1.html` visual direction: green palette, compact sidebar, header dropdowns, and calendar layout.
